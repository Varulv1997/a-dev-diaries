...
var jscolor = {
  ...
  install : function() { jscolor.addEvent(window, 'load', jscolor.init); },
  ...
  bind : function() {
    var matchClass =
        new RegExp('(^|\\s)(' + jscolor.bindClass + ')\\s*(\\{[^}]*\\})?', 'i');
  ...
  requireImage : function(filename) { jscolor.imgRequire[filename] = true; },
  ...
  color : function(target, prop) {
    ...
    this.onImmediateChange =
        null; // onchange callback (can be either string or function)
    ...
    this.pickerSmartPosition =
        true; // automatically adjust picker position when necessary
    ...
    this.pickerBorderColor =
        'ThreeDHighlight ThreeDShadow ThreeDShadow ThreeDHighlight'; // CSS
                                                                     // color
    this.pickerInset = 1;                                            // px
    this.pickerInsetColor =
        'ThreeDShadow ThreeDHighlight ThreeDHighlight ThreeDShadow'; // CSS
                                                                     // color
    ...
    this.fromRGB = function(r, g, b, flags) { // null = don't change
      ...
      var hsv =
          RGB_HSV(r === null ? this.rgb[0] : r, g === null ? this.rgb[1] : g,
                  b === null ? this.rgb[2] : b);
      if (hsv[0] !== null) {
        this.hsv[0] =
            Math.max(0.0, this.minH, Math.min(6.0, this.maxH, hsv[0]));
      }
      if (hsv[2] !== 0) {
        this.hsv[1] =
            hsv[1] === null
    ...
    function drawPicker(x, y) {
      ...
      if ('ontouchstart' in window) { // if touch device
        p.box.addEventListener('touchmove', function(e) {
          ...
          e.preventDefault();  // prevent Default - Android Fix (else android
                               // generated only 1-2 touchmove events)
        }, false);
      }
      ...
      p.padM.style.width = THIS.pickerFace + 2 * THIS.pickerInset +
                           jscolor.images.pad[0] + jscolor.images.arrow[0] +
                           'px';
      ...
      p.btn.onmousedown = function() { THIS.hidePicker(); };
    ...
    function redrawPad() {
      ...
      var y =
          Math.round((1 - THIS.hsv[yComponent]) * (jscolor.images.pad[1] - 1));
    ...
    function redrawSld() {
      ...
      var y =
          Math.round((1 - THIS.hsv[yComponent]) * (jscolor.images.sld[1] - 1));
    ...
    function setPad(e) {
      ...
      case 0:
        ...
        break;
      case 1:
        THIS.fromHSV(x * (6 / (jscolor.images.pad[0] - 1)), null,
                     1 - y / (jscolor.images.pad[1] - 1), leaveSld);
        break;
      }
    }
    ...
    var leaveValue = 1 << 0, leaveStyle = 1 << 1, leavePad = 1 << 2,
        leaveSld = 1 << 3;
