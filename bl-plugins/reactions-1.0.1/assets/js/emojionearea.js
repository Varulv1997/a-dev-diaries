...
function trigger(self, event, args) {
  ...
  if (event) {
    ...
    do {
      ...
      if (eventStorage[self.id][_event] &&
...
function attach(self, element, events, target) {
  target =
      target || function(event,
                         callerEvent) { return $(callerEvent.currentTarget) };
...
function getTemplate(template, unicode, shortname) {
  ...
  template =
      template.replace('{name}', shortname || '')
  ...
  if (shortname) {
    template =
        template.replace('{alt}', emojione.shortnameToUnicode(shortname));
  } else {
...
function getEmojioneVersion() { return window.emojioneVersion || '3.1.2'; };
function isObject(variable) { return typeof variable === 'object'; };
...
function getDefaultOptions() {
  ...
  var supportMode = !emojione ? getSupportMode(getEmojioneVersion())
...
function getOptions(options) {
  ...
  if (options && options['filters']) {
    ...
    $.each(options['filters'], function(filter, data) {
      ...
      $.each(data, function(key, val) { filters[filter][key] = val; });
    });
...
if (window.getSelection && document.createRange) {
  ...
  saveSelection = function(el) { return document.selection.createRange(); };
...
function div(prefix) {
  var parent = $(
      '<div/>', isObject(prefix) ? prefix : {"class" : selector(prefix, true)});
...
function updateRecent(self, show) {
  ...
  if (!self.recent || self.recent !== emojis || show) {
    if (emojis.length) {
      var skinnable = self.scrollArea.is(".skinnable"), scrollTop, height;
      ...
      var items =
          shortnameTo(emojis, self.emojiBtnTemplate, true).split('|').join('');
      self.recentCategory.children(".emojibtn").remove();
      $(items).insertAfter(
          self.recentCategory.children(".emojionearea-category-title"));

      self.recentCategory.children(".emojibtn")
          .on("click", function() { self.trigger("emojibtn.click", $(this)); });
...
function init(self, source, options) {
  ...
  $.each(options.attributes,
         function(attr, value) { editor.attr(attr, value); });

  var mainBlock =
      div('category-block').attr({"data-tone" : 0}).prependTo(emojisList);
  ...
  $.each(options.filters, function(filter, params) {
    ...
    do {
      var category, items = params.emoji.replace(/[\s,;]+/g, '|');
  ...
  // 3) bindEvent() finds events in possibleEvents{} and bind founded via
  // jQuery.on() 4) attached events via jQuery.on() calls trigger() 5) trigger()
  // calls handlers stored into eventStorage{}
  ...
  attach(self, editor, [ "focus", "blur" ],
         function() { return self.stayFocused ? false : editor; });
  attach(self, picker, {
  ...
  scrollArea.on('scroll', function() {
    if (!noListenScroll) {
      ...
      if (scrollArea.is(":not(.skinnable)")) {
        ...
        var filter =
            filtersBtns.filter('[data-filter="' + item.attr("name") + '"]');
  ...
  self.on("@filter.click",
          function(filter) {
            ...
            var headerOffset =
                    categories.filter('[name="' + filter.data('filter') + '"]')
                        .offset()
                        .top,
            ...
            scrollArea.stop().animate(
                {scrollTop : headerOffset + scroll - offsetTop - 2}, 200,
                'swing', function() {
                  lazyLoading.call(self);
      ...
      .on("@tone.click",
          function(tone) {
            ...
            if (skin) {
              ...
              filtersBtns.removeClass(
                  "active"); //.not('[data-filter="recent"]').eq(0).addClass("active");
            } else {
      ...
      .on("@change",
          function() {
            var html =
                self.editor.html().replace(/<\/?(?:div|span|p)[^>]*>/ig, '');
      ...
      .on("@focus", function() { app.addClass("focused"); })
    ...
    self.on("@search.focus",
        ...
        .on("@search.keypress",
            function(hide) {
              ...
              var activeTone =
                  (options.tones ? tones.find("i.active").data("skin") : 0);
              var term =
                  self.search.val().replace(/ /g, "_").replace(/"/g, "\\\"");

              if (term && term.length) {
                if (self.recentFilter.hasClass("active")) {
                  self.recentFilter.removeClass("active").next().addClass(
                      "active");
                }
                ...
                categoryBlocks.each(function() {
                  var matchEmojis =
                      function(category, activeTone) {
                    var $matched =
                        category.find('.emojibtn[data-name*="' + term + '"]');
                    if ($matched.length === 0) {
                  ...
                  if ($block.data('tone') === 0) {
                    categories.filter(':not([name="recent"])')
                        .each(function() { matchEmojis($(this), 0); })
                  } else {
  ...
  if (isObject(options.events) && !$.isEmptyObject(options.events)) {
    $.each(options.events,
           function(event,
                    handler) { self.on(event.replace(/_/g, '.'), handler); });
  }
    ...
    var autocomplete = function() {
      ...
      var map = $.map(emojione.emojioneList, function(_, emoji) {
        return !options.autocompleteTones
                   ? /_tone[12345]/.test(emoji) ? null : emoji
                   : emoji;
      });
      map.sort();
      editor.textcomplete(
          [ {
            ...
            replace : function(
                value) { return shortnameTo(value, self.emojiTemplate); },
            cache : true,
            index : 1
          } ],
          textcompleteOptions);
      ...
      if (options.textcomplete.placement) {
        ...
        if ($(editor.data('textComplete').option.appendTo).css("position") ==
            "static") {
          $(editor.data('textComplete').option.appendTo)
      ...
      $.ajax({
        ...
        success : function() {
          ...
          if (emojioneSupportMode > 4) {
            ...
            sprite = cdn.base + "/sprites/emojione-sprite-" +
                     emojione.emojiSize + ".css";
          } else {
...
var EmojioneArea = function(element, options) {
  ...
  emojioneReady(function() { init(self, element, options); });
};
...
EmojioneArea.prototype.off = function(events, handler) {
  if (events) {
    var id = this.id;
    $.each(events.toLowerCase().replace(/_/g, '.').split(' '),
           function(i, event) {
...
EmojioneArea.prototype.setFocus = function() {
  var self = this;
  emojioneReady(function() { self.editor.focus(); });
  return self;
};

EmojioneArea.prototype.setText =
    function(str) {
  var self = this;
  emojioneReady(function() {
  ...
  return self;
    ...
    EmojioneArea.prototype.getText =
        function() { return textFromHtml(this.editor.html(), this); }

        EmojioneArea.prototype.showPicker =
            function() {
  var self = this;
  if (self._sh_timer) {
  ...
  self._sh_timer =
      window.setTimeout(function() { self.button.addClass("active"); }, 50);
  trigger(self, "picker.show", [ self.picker ]);
  return self;
}

            EmojioneArea.prototype.hidePicker =
                function() {
  var self = this;
  if (self._sh_timer) {
    window.clearTimeout(self._sh_timer);
  ...
  self._sh_timer =
      window.setTimeout(function() { self.picker.addClass("hidden"); }, 500);
  trigger(self, "picker.hide", [ self.picker ]);
  return self;
}

                EmojioneArea.prototype.enable =
                    function() {
  var self = this;
  var next = function() {
    self.disabled = false;
  ...
  return self;
                    ...
                    EmojioneArea.prototype.disable =
                        function() {
  var self = this;
  self.disabled = true;
  var next = function() {
...
$.fn.emojioneAreaText = function(options) {
  ...
  var self = this, pseudoSelf = {
    shortnames : (options && typeof options.shortnames !== 'undefined'
                      ? options.shortnames
                      : true),
    emojiTemplate : '<img alt="{alt}" class="emojione' +
  ...
  emojioneReady(function() {
    self.each(function() {
      ...
      if (!$this.hasClass('emojionearea-text')) {
        $this.addClass('emojionearea-text')
            .html(htmlFromText(($this.is('TEXTAREA') || $this.is('INPUT')
                                    ? $this.val()
                                    : $this.text()),
                               pseudoSelf));
      }
